...
var WorkerSourceContextMaxSize = dynamicconfig.NewNamespaceIntSetting(
	"callback.worker.sourceContext.maxSize",
	64*1024,
	`The maximum allowed size, in bytes, of the opaque source context attached to a single Worker
completion callback. The server carries this payload to the callback's handler untouched.`,
)

var WorkerSourceContextAggregateMaxSize = dynamicconfig.NewNamespaceIntSetting(
	"callback.worker.sourceContext.aggregateMaxSize",
	2*1024*1024,
	`The maximum allowed total size, in bytes, of the source context payloads carried by all Worker
